...
require_once('library/fathom-post-type.php'); // you can disable this if you like
...
add_image_size( 'bones-thumb-493', 493, 197, false );
/*
...
function bones_deregister_sidebars() {

	unregister_sidebar( 'sidebar1' );
	unregister_sidebar( 'homepagehighlight' );
	unregister_sidebar( 'social_media' );

}
add_action( 'widgets_init', 'bones_deregister_sidebars', 11 );
...
// Replace more text after both manual and automatic excerpts

function custom_excerpt($text) {
   if (strpos($text, 'Read more &raquo;')) {
	  // for generated excerpts
      $excerpt = strip_tags(str_replace('Read more &raquo;', '<a href="' . get_permalink() . '">more &gt;</a>', $text), "<a>");
   } else {
	  // for manual excerpts
...
// Hide crazy and confusing Custom Fields section in admin
function customize_meta_boxes() {
     remove_meta_box('postcustom','post','normal');
	 remove_meta_box('postcustom','bio','normal');
	 remove_meta_box('postcustom','news_intel','normal');
	 remove_meta_box('postcustom','publications','normal');
	 remove_meta_box('postcustom','video','normal');
	 remove_meta_box('postcustom','fathom','normal');
